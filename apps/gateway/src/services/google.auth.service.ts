...
import { IGoogleToken } from '../interfaces/google-token.interface';
...
export class GoogleAuthService {
  ...
  private async fetchGoogleOAuthTokens(code: string): Promise<IGoogleToken> {
    ...
    const response = await firstValueFrom(
      this.httpService
        ...
        .pipe(map((response: AxiosResponse<IGoogleToken>) => response.data)),
    );
  ...
  private async fetchGoogleUser(
    ...
    const response = await firstValueFrom(
      this.httpService
        .get<IGoogleUser>(`${this.googleApiUrl}=${access_token}`, {
          headers: { Authorization: `Bearer ${id_token}` },
        })
        .pipe(map((response: AxiosResponse<IGoogleUser>) => response.data)),
    );
